...
import { getTVShowDetails, getBackdropUrl } from "@/services/tmdb";
import { generateRoomId } from "@/lib/mock-data";
import { useAuth } from "@/hooks/use-auth";
import type { TMDBTVShowDetails } from "@/types/tmdb";

interface WatchTogetherTVPageProps {
  params: Promise<{ id: string }>;
}

export default function WatchTogetherTVPage({
  params,
}: WatchTogetherTVPageProps) {
  const { id } = use(params);
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR<TMDBTVShowDetails>(`tv-${tvId}`, () => getTVShowDetails(tvId));
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/watch-together/tv/${tvId}?room=${roomId}`
      : "";
  ...
  const avgRuntime = tvShow.episode_run_time?.[0] || 0;
      ...
      <div className="container mx-auto px-4 -mt-20 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6 min-w-0">
            <VideoPlayer
              title={tvShow.name}
              posterUrl={getBackdropUrl(tvShow.backdrop_path, "w1280")}
            />
          ...
          <div className="lg:w-80 shrink-0 space-y-4">
            <h1 className="text-2xl md:text-3xl font-bold">{tvShow.name}</h1>
